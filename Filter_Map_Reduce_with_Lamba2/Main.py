...
def main():
	...
	ret=list(filter(lambda no: (no%2==0),List))     #ret=Filterr(List)

	print("The filtered list is:",ret)

	
	ret1=list(map(lambda no: no+2,ret))    #ret1=Map(ret)

	print("The mapped list is:",ret1)

	ret2=functools.reduce(lambda no1,no2:no1+no2,ret1)		#ret2=Reduce(ret1) need to import a specific module for Python3
